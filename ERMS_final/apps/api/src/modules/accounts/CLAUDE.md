...
- `listPendingPayments` — every `ACCOUNTS_VERIFIED` claim, i.e. "pending
  payable". A claim only leaves this list via `processPayment`, so nothing
  verified can be silently skipped — it's compulsorily either paid or still
  sitting here.
