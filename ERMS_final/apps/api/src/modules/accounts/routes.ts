...
// GET /api/accounts/payments — Payment Processing screen: every verified
// claim not yet paid ("pending payable")
accountsRouter.get("/payments", async (_req, res, next) => {
  try {
    const claims = await accountsService.listPendingPayments();
    res.json({ data: claims });
  } catch (err) {
    next(err);
  }
