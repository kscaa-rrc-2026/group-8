...
const NAV_BY_ROLE: Record<Role, NavItem[]> = {
  ...
  // CEO submits claims like any employee, and self-approves them (no one
  // sits above the CEO to check their claim) via the same Approval Queue.
  CEO: [
    { to: "/manager/approvals", label: "Approval Queue" },
    { to: "/manager/summary", label: "Approval Summary" },
    { to: "/employee/claims/new", label: "New Claim" },
...
// Available to every role regardless of what's above — always the last item.
const ACCOUNT_NAV: NavItem = { to: "/change-password", label: "Change Password" };
  ...
  return (
    <aside className="w-60 shrink-0 bg-brand-900 text-brand-100 min-h-screen p-4">
      ...
      <nav className="flex flex-col gap-1">
        {[...NAV_BY_ROLE[role], ACCOUNT_NAV].map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            className={({ isActive }) =>
