...
export function ClaimReview() {
  ...
  // send — fetch the file as a blob with the authenticated client instead
  // and open it from an in-memory object URL.
        ...
        <div className="mt-4">
          ...
          <ul className="text-sm space-y-1">
            {claim.attachments.map((a) => (
              <li key={a.id}>
                <button type="button" onClick={() => previewAttachment(a.id)} className="text-brand-700 underline hover:text-brand-900">
                  {a.fileName}
                </button>
              </li>
            ))}
